...
import {
  ...
  removeToCart,
  removeProduct,
} from "../features/addToCart/addCartSlice";
...
const Sidebar = () => {
  ...
  return (
    <div
      className={`${
        sideBar ? "w-full" : "w-0"
      } overflow-hidden duration-300 transition-all bg-black/[0.7] z-30 right-0
     top-0 h-full 
    fixed`}
    >
      <div
        className="h-full overflow-x-hidden overflow-y-scroll bg-white w-80 absolute right-0 duration-300 transition-all
        ...
        <div className="h-full w-full px-2 pb-10 overflow-scroll">
          {cartNumber.length >= 1 ? (
            products.map((product) => (
              <div
                key={product.id}
                className="flex py-3 items-center w-full justify-between px-2 shadow-md"
              >
                <img
                  src={product.image}
                  className="object-contain w-28 h-32 me-2"
                  alt=""
                />
                <div>
                  ...
                  <p className="mb-2 w-32 text-xs text-black/[0.5]">
                  ...
                  >
                    ...
                    <div className="bg-black/[0.1] w-full flex items-center justify-center">
                      {product.quantity}
                    </div>
                    <button
                      onClick={() => handleRemoveCart(product)}
                      className="font-semibold border-l flex items-center
                       justify-center px-2"
                    >
                      -
                    </button>
            ...
            <div className="flex flex-col items-center justify-center gap-4 ">
              ...
              <Link
                onClick={handleSidebar}
                to={"/"}
                className="mt-10 w-full flex items-center justify-center text-white bg-black px-4 py-2 rounded-lg hover:opacity-75 active:scale-95"
              >
                Continue shopping
              </Link>
            </div>
          ...
          <Link
            onClick={handleSidebar}
            to={"/cart"}
            className="mt-10 text-white bg-black w-full flex items-center justify-center px-4 py-2 rounded-lg hover:opacity-75 active:scale-95"
          >
            CheckOut
          </Link>
        </div>
